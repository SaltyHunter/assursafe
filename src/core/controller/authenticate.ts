...
api.post('/signup', async (req: Request, res: Response) => {
  const fields = ['username', 'mail', 'password', 'passwordConfirmation']
  ...
  try {
    ...
    const { nickname, mail, password, passwordConfirmation } = req.body

    if (password !== passwordConfirmation) {
      throw new Error("Password doesn't match")
    }
    await sendConfirmation(mail, { nickname })
